...
class ApiClient {
  ...
  async login(username: string, password: string): Promise<ApiResponse<{ token: string; admin: ApiUser }>> {
    return this.request<ApiResponse<{ token: string; admin: ApiUser }>>('/login', {
      method: 'POST',
  ...
  async getEmployees(params?: {
    ...
    return this.request<PaginatedResponse<{ employees: ApiEmployee[] }>>(endpoint);
  }
...
export const transformApiEmployeeToCrudItem = (apiEmployee: ApiEmployee): CrudItem => ({
  id: apiEmployee.id,
  name: apiEmployee.name,
  email: '', // Not provided by API
  phone: apiEmployee.phone,
  position: apiEmployee.position,
  department: apiEmployee.division.name,
  division_id: apiEmployee.division.id,
